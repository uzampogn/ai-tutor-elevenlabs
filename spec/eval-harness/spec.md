...
- Implementation notes (Task 10 — `scripts/eval/run.ts`):
  ...
  - **Baseline-drop gating added** (blessed by the task, spec gap): after printing the diff table, the runner fails the gate (`console.error` + exit 1, alongside the tolerance-based regression check) if any metric present in the baseline file is absent from the current aggregates — which `diffAgainstBaseline` would otherwise silently drop and never gate. A metric that stops being emitted is a regression. No change to `baseline.ts`.
  ...
  - **Deviation (live round): `maxConcurrency: 1` (plan said 3) plus a 25s inter-item throttle.** The Voyage API key is free-tier (3 requests/min); each eval item does one query-embed. Serialization alone proved insufficient in the first live attempt — fast items (e.g. offtopic → short answer, quick judge) cycle in <20s and trip HTTP 429, which `retrieveArticles` silently degrades to `[]`, contaminating recall/mrr and inflating offtopic_pass. `run.ts` therefore also enforces a hard 25s floor between item starts (~2.4 embeds/min).
  - **Live run BLOCKED (environmental, no code impact — resolved in live round 2):** `DATABASE_URL`/`POSTGRES_URL` is absent from `.env.local` in both worktrees, and no Vercel CLI/auth is available locally to pull it (the `VERCEL_OIDC_TOKEN` is runtime-only). Retrieval reads Supabase pgvector, so without the URL every item retrieves `[]` (recall/mrr ≡ 0), which would produce a degenerate, misleading baseline candidate and fail Step-4 verification (`recall/mrr > 0`). Rather than spend the single-run budget on that, the run was not performed. The runner's env guard was exercised end-to-end (Node 22): `npm run eval` exits 1 with `[eval] DATABASE_URL or POSTGRES_URL required (retrieval reads pgvector)` before any API call (zero tokens). Offline gate (lint + typecheck + test:run, 482 tests) is green.
  - **Live round 2 — first executed run; baseline deliberately NOT blessed.** With a working `POSTGRES_URL`, run `eval-f9a6b46-1784396375425` completed 26/26 items (exit 0, "no baseline yet" notice). Two runner fixes landed first: `prepareAnswerContext` falls back to the uncached grounding build when `unstable_cache` throws its "incrementalCache missing" invariant outside the Next runtime (without this, every item failed before generation), and the 25s inter-item throttle above (4 Voyage 429s still contaminated 4 items in this run: 3 on-topic items degraded to empty retrieval, recall/mrr ≡ 0; 1 offtopic item false-passed). The decisive finding: **all 3 uncontaminated offtopic questions retrieved 3 articles each at `SIM_FLOOR = 0.35`** (`retrieval.offtopic_pass` ≤ 0.25 true value vs the ≥ 0.5 pre-bless sanity floor). The floor is too low for voyage-3.5-lite similarity ranges — exactly the tuning the constant's "starting guess" comment anticipated. Blessing this run would encode that behavior into the baseline, so `eval/baseline.json` is withheld until `SIM_FLOOR` is re-tuned (product decision); the regression-gate DoD proof is deferred with it.
  - **Live round 3 — first blessed baseline (policy decision).** The initial `eval/baseline.json` (run `eval-ef0f9db-1784398197360`, clean: 0 × 429, failedCount 0) records `retrieval.offtopic_pass = 0.25` deliberately: it documents the known `SIM_FLOOR = 0.35` off-topic leak (beads MP-gm5) as current truth rather than gating on it — the future SIM_FLOOR fix will be proven as an improvement against this baseline.
